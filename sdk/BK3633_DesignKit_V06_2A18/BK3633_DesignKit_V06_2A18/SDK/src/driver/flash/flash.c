...
// 2025/6/5  verify by yubei,for GT/PY/TH flash
void flash_write_data (uint8_t *buffer, uint32_t address, uint32_t len)
{
    ...
    while(len) 
    {
      //  if((address & 0x1F) || (len < 32))
        ...
        if(flash_mid==P25Q40U) // sonos flash,the soft earse is before write,so the data must be rewrite at the same
        {
            for(i =0;i< 32;  i++) 
            {
                if((i>=(address & 0x1F))&&(i<((address & 0x1F)+len)))
                {
        ...
        else// such as TH & GT,if the bit is 0,don't rewrite 0 in this bit 
        {
            for(i =0;i< 32;  i++) 
            {
                if((i>=(address & 0x1F))&&(i<((address & 0x1F)+len)))
